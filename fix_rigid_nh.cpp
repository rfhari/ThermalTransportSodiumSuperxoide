  ...
  id_press(nullptr), temperature(nullptr), pressure(nullptr)
{
  ...
  fname3<<"flux_na_data_"<<id<<".txt";
  nadata.open(fname3.str());
  
  std::ostringstream fname4;
  fname4<<"flux_oxy_data_"<<id<<".txt";
  oxydata.open(fname4.str());
  ...
  for(int ibody=0; ibody< nbody; ibody++){
    for(int jbody=0; jbody< nbody; jbody++){
      ...
      for(int idir=0; idir<3; idir++){
        ...
        if (ibody<256){
          flux_oxy[9+idir]+= (0.5 * Rij[idir]*(vcm[ibody][0]*final_force[ibody][jbody][0]+\
                              vcm[ibody][1]*final_force[ibody][jbody][1]+\
                              vcm[ibody][2]*final_force[ibody][jbody][2]));
          flux_oxy[12+idir]+= (0.5 * Rij[idir]*(omega[ibody][0]*final_torque[ibody][jbody][0]+\
                              omega[ibody][1]*final_torque[ibody][jbody][1]+\
                              omega[ibody][2]*final_torque[ibody][jbody][2]));
        }
        if (ibody>255){
          flux_na[9+idir]+= (0.5 * Rij[idir]*(vcm[ibody][0]*final_force[ibody][jbody][0]+\
                              vcm[ibody][1]*final_force[ibody][jbody][1]+\
                              vcm[ibody][2]*final_force[ibody][jbody][2]));
          flux_na[12+idir]+= (0.5 * Rij[idir]*(omega[ibody][0]*final_torque[ibody][jbody][0]+\
                              omega[ibody][1]*final_torque[ibody][jbody][1]+\
                              omega[ibody][2]*final_torque[ibody][jbody][2]));
        }
  ...
  if (1==dump_body_flag){
    for(int ibody=0; ibody< nbody; ibody++){
      if (ibody<256){
        ...
        //body_properties<< final_force[0][ibody][0]<<" " << final_force[0][ibody][1]<<" "<< final_force[0][ibody][2]<< " ";
        body_properties<<endl;
      }
    }
  body_properties<<endl;
